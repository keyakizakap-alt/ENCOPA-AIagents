...
分析が失敗したときの原因の種別は、次の3か所で確認できます。**画面には出しません**（利用者には関係のない情報のため）。

1. `/api/agent` の応答の `reason`
2. ブラウザの開発者ツールのコンソール（`[encopa] agent unavailable`）
3. サーバーログの `agent_workflow_failed` の `reason`

2 はデプロイ済みの画面でそのまま確認できます。ホスティング先のログ権限が無くても切り分けられます。
...
- 分析が失敗したとき、画面に技術的な情報（reason・エラーID）が出ていない
