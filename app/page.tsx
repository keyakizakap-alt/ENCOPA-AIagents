      ...
      <div className="min-w-0">
        ...
        <div className="overflow-hidden rounded-[28px] border border-[#1e2928]/10 bg-[#1f4b46] shadow-[0_18px_60px_rgba(31,75,70,.14)]">
          <div className="grid gap-7 p-5 sm:p-7 xl:grid-cols-[1fr_280px] xl:p-9">
            <div><div className="mb-5 flex items-center gap-2 text-[#d9c9a7]"><Sparkles className="size-4"/><span className="text-[13px] font-semibold tracking-[.08em]">集まる日の準備を、ひとつに</span></div><h1 className="jp-text max-w-[680px] font-serif text-[clamp(1.75rem,3.3vw,3rem)] leading-[1.14] tracking-[-.03em] text-[#fffaf1] sm:leading-[1.04] sm:tracking-[-.045em]">{/* Japanese wraps at any character by default, which split 条件を変えるた/び、 mid-word.
              Each phrase is its own inline-block, so a line can only end where a reader would pause. */}
              <span className="inline-block">条件を変えるたび、</span><br className="hidden sm:block"/><span className="inline-block">候補と理由を<wbr/>組み直します。</span></h1><p className="mt-4 max-w-2xl text-[15px] leading-7 text-[#e5e8df]/75">候補を比べて、予約内容をみんなで共有。アレルギーの確認も、待ち合わせの連絡も、この会のグループで。</p></div>
          ...
          <div className="grid gap-3 border-t border-white/10 bg-[#163d39] p-4 sm:grid-cols-2 sm:p-5 lg:grid-cols-3 2xl:grid-cols-[repeat(5,minmax(0,1fr))_auto] 2xl:items-end 2xl:gap-4">
            <Field label="目的"><Select value={purpose} onValueChange={setPurpose}>{/* data-[size=default]:h-12 is needed because the component sets its height through a
              ...
              <SelectTrigger className="h-12 w-full rounded-xl border-white/10 bg-white px-3 text-base font-medium text-[#1e2928] data-[size=default]:h-12"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="忘年会">忘年会</SelectItem><SelectItem value="新年会">新年会</SelectItem><SelectItem value="歓迎会">歓迎会</SelectItem><SelectItem value="送別会">送別会</SelectItem><SelectItem value="懇親会">懇親会</SelectItem><SelectItem value="打ち上げ">打ち上げ</SelectItem></SelectContent></Select></Field>
            <Field label="場所"><button type="button" onClick={()=>setLocationOpen(true)} className="flex h-12 w-full items-center justify-between rounded-xl border border-white/10 bg-white px-3 text-left text-base font-medium text-[#1e2928] transition hover:bg-[#f7f5ef]"><span className="flex min-w-0 items-center gap-2"><MapPin className="size-4 shrink-0 text-[#687370]"/><span className="truncate">{area}</span></span><ChevronRight className="size-4 shrink-0 text-[#687370]"/></button></Field>
            <Field label="予算 / 人"><div className="relative"><Input inputMode="numeric" value={budget} onChange={e=>setBudget(e.target.value.replace(/\D/g,""))} className="h-12 rounded-xl border-white/10 bg-white px-3 pr-9 text-base font-medium text-[#1e2928]"/><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-[#687370]">円</span></div></Field>
            <Field label="人数"><div className="relative"><Input inputMode="numeric" value={people} onChange={e=>setPeople(e.target.value.replace(/\D/g,""))} className="h-12 rounded-xl border-white/10 bg-white px-3 pr-9 text-base font-medium text-[#1e2928]"/><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-[#687370]">名</span></div></Field>
            <Field label="アレルギー"><button type="button" onClick={()=>setAllergyOpen(true)} className="flex h-12 w-full items-center justify-between rounded-xl border border-white/10 bg-white px-3 text-left text-base font-medium text-[#1e2928] transition hover:bg-[#f7f5ef]"><span className="truncate">{hasAllergy?`${allergy.items.length}項目を設定`:allergy.status==="none"?"なし":"設定する"}</span><ChevronRight className="size-4 shrink-0 text-[#687370]"/></button></Field>
            <div className="flex items-end"><Button disabled={searching} aria-busy={searching} onClick={()=>void search()} className="h-12 w-full rounded-xl bg-[#e17a4e] px-6 text-base font-semibold text-white shadow-lg transition hover:bg-[#ee8a5e] 2xl:w-auto">{searching?<RefreshCw className="mr-2 size-4 animate-spin"/>:<Search className="mr-2 size-4"/>}{searching?"検索中":"実店舗を検索"}</Button></div>
          </div>
        </div>
