...
import { AppShell, type ShellSection } from "@/components/encopa/app-shell";
...
export default function Home() {
  ...
  const downloadCalendar=()=>{
    ...
    // Date.toISOString() throws on an invalid date, which aborted the click handler
  ...
  useEffect(()=>{
    try {
      ...
      if(saved){
        ...
        const restored=restoredAllergy(value.allergy);
        if(restored)setAllergy(restored);
      }
    } catch {}
    setRestored(true);
  },[]);
  ...
  // Navigating scrolls to the matching region: one page, five entry points, so a tap from
  // the tab bar never lands on an empty route.
  const goTo=(next:ShellSection)=>{
    setSection(next);
    const anchor={home:"top",venues:"results",participants:"plan-share",agent:"agent-panel",settings:"top"}[next];
    if(next==="settings"){setDraftPriority(priority);setDraftPrivateRoom(privateRoom);setDraftDietary(dietary);setDraftEventDate(eventDate);setDraftEventTime(eventTime);setSettingsOpen(true);return}
    document.getElementById(anchor)?.scrollIntoView({behavior:"smooth",block:"start"});
  };

  return <AppShell active={section} onNavigate={goTo}><main id="top" className="min-h-screen bg-[#fdf6ef] text-[#211f1d]">
    <header className="sticky top-0 z-40 border-b border-[#211f1d]/10 bg-[#fdf6ef]/92 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1440px] flex-wrap items-center gap-x-5 gap-y-2 px-4 py-3 sm:px-7 lg:px-8">
        <div className="flex items-center gap-3 lg:hidden"><div className="grid size-9 place-items-center rounded-[12px] bg-[#d03e28] text-[#fff7f0]"><UtensilsCrossed className="size-[18px]"/></div><p className="text-[18px] font-black tracking-[.08em]">ENCOPA</p></div>
        <div className="flex min-w-0 items-center gap-2">
          <CalendarDays className="size-[18px] shrink-0 text-[#c03429]"/>
          <h1 className="truncate text-[17px] font-bold">{query.purpose}{eventDate.slice(0,4)}</h1>
          <span className="shrink-0 rounded-full bg-[#fdeae5] px-2.5 py-1 text-[11px] font-semibold text-[#c03429]">{stage==="scheduled"?"予定確保ずみ":stage==="draft"?"下書き":"進行中"}</span>
        </div>
        {/* 会の骨子は常に目に入る位置へ。狭い画面では折り返して全項目を残す。 */}
        <dl className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1 text-[13px] text-[#6b635c]">
          <div className="flex items-center gap-1.5"><Clock3 className="size-4 shrink-0"/><dt className="sr-only">日時</dt><dd>{eventDate.replaceAll("-","/")} {eventTime}〜</dd></div>
          <div className="flex items-center gap-1.5"><Users className="size-4 shrink-0"/><dt className="sr-only">人数</dt><dd>{query.people}名</dd></div>
          <div className="flex items-center gap-1.5"><WalletCards className="size-4 shrink-0"/><dt className="sr-only">予算</dt><dd>{chosen?chosen.budgetLabel:`〜${query.budget.toLocaleString()}円`} / 人</dd></div>
          <div className="flex items-center gap-1.5"><MapPin className="size-4 shrink-0"/><dt className="sr-only">場所</dt><dd className="max-w-[10rem] truncate">{query.area}</dd></div>
        </dl>
        <div className="ml-auto flex shrink-0 items-center gap-2">
          <Button disabled={!chosen} onClick={()=>void sharePlan()} variant="outline" className="h-9 rounded-full border-[#211f1d]/15 bg-white px-4"><Share2 className="mr-2 size-4"/>共有</Button>
          <Button onClick={()=>{setDraftPriority(priority);setDraftPrivateRoom(privateRoom);setDraftDietary(dietary);setDraftEventDate(eventDate);setDraftEventTime(eventTime);setSettingsOpen(true)}} variant="outline" className="h-9 rounded-full border-[#211f1d]/15 bg-white px-4"><Settings2 className="mr-2 size-4"/><span className="hidden sm:inline">詳細設定</span></Button>
        </div>
      </div>
    </header>
    ...
    <section className="mx-auto grid max-w-[1440px] gap-6 px-4 py-5 sm:px-7 lg:grid-cols-[minmax(0,1fr)_340px] lg:px-10 lg:py-8">
      <div className="min-w-0">
        {/* The stepper scrolls sideways on narrow screens; the fade shows there is more to see
            instead of leaving the last chip looking clipped. */}
        <div className="mb-5 flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none [mask-image:linear-gradient(to_right,#000_0,#000_calc(100%-2.5rem),transparent_100%)] lg:[mask-image:none]">
          {steps.map(([label],i)=><div key={label} className="flex shrink-0 items-center gap-2"><div className={`flex items-center gap-2 rounded-full px-3 py-2 text-[13px] font-medium ${i<=stageIndex?"bg-[#d03e28] text-white":"bg-white text-[#6b635c] ring-1 ring-[#211f1d]/10"}`}>{i<stageIndex?<Check className="size-3.5"/>:<span className="grid size-4 place-items-center rounded-full bg-current/10 text-[10px]">{i+1}</span>}{label}{i===stageIndex&&<span className="text-[10px] opacity-65">いまここ</span>}</div>{i<steps.length-1&&<ChevronRight className="size-4 text-[#cbbfb2]"/>}</div>)}
        </div>

        <div className="overflow-hidden rounded-[28px] border border-[#211f1d]/10 bg-[#fffaf5] shadow-[0_18px_50px_rgba(33,31,29,.07)]">
          <div className="grid gap-7 p-5 sm:p-7 xl:grid-cols-[1fr_280px] xl:p-9">
            <div><div className="mb-5 flex items-center gap-2 text-[#c03429]"><Sparkles className="size-4"/><span className="text-[13px] font-semibold tracking-[.08em]">集まる日の準備を、ひとつに</span></div><h1 className="jp-text max-w-[680px] font-serif text-[clamp(1.6rem,2.6vw,2.5rem)] leading-[1.14] tracking-[-.03em] text-[#211f1d] sm:leading-[1.04] sm:tracking-[-.045em]">{/* Japanese wraps at any character by default, which split 条件を変えるた/び、 mid-word.
              Each phrase is its own inline-block, so a line can only end where a reader would pause. */}
          ...
          <div className="grid gap-3 border-t border-[#e3d5c6] bg-[#f6ece1] p-4 sm:grid-cols-2 sm:p-5 lg:grid-cols-3 2xl:grid-cols-[repeat(5,minmax(0,1fr))_auto] 2xl:items-end 2xl:gap-4">
            ...
            <Field label="場所"><button type="button" onClick={()=>setLocationOpen(true)} className="flex h-12 w-full items-center justify-between rounded-xl border border-[#e3d5c6] bg-white px-3 text-left text-base font-medium text-[#211f1d] transition hover:bg-[#fdf6ef]"><span className="flex min-w-0 items-center gap-2"><MapPin className="size-4 shrink-0 text-[#6b635c]"/><span className="truncate">{area}</span></span><ChevronRight className="size-4 shrink-0 text-[#6b635c]"/></button></Field>
            <Field label="予算 / 人"><div className="relative"><Input inputMode="numeric" value={budget} onChange={e=>setBudget(e.target.value.replace(/\D/g,""))} className="h-12 rounded-xl border-[#e3d5c6] bg-white px-3 pr-9 text-base font-medium text-[#211f1d]"/><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-[#6b635c]">円</span></div></Field>
            <Field label="人数"><div className="relative"><Input inputMode="numeric" value={people} onChange={e=>setPeople(e.target.value.replace(/\D/g,""))} className="h-12 rounded-xl border-[#e3d5c6] bg-white px-3 pr-9 text-base font-medium text-[#211f1d]"/><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-[#6b635c]">名</span></div></Field>
            <Field label="アレルギー"><button type="button" onClick={()=>setAllergyOpen(true)} className="flex h-12 w-full items-center justify-between rounded-xl border border-[#e3d5c6] bg-white px-3 text-left text-base font-medium text-[#211f1d] transition hover:bg-[#fdf6ef]"><span className="truncate">{hasAllergy?`${allergy.items.length}項目を設定`:allergy.status==="none"?"なし":"設定する"}</span><ChevronRight className="size-4 shrink-0 text-[#6b635c]"/></button></Field>
            <div className="flex items-end"><Button disabled={searching} aria-busy={searching} onClick={()=>void search()} className="h-12 w-full rounded-xl bg-[#ef8354] px-6 text-base font-semibold text-white shadow-lg transition hover:bg-[#f2966a] 2xl:w-auto">{searching?<RefreshCw className="mr-2 size-4 animate-spin"/>:<Search className="mr-2 size-4"/>}{searching?"検索中":"実店舗を検索"}</Button></div>
          </div>
        </div>
        ...
        <section id="results" className="scroll-mt-24 pt-8">
          ...
          {allergy.status==='selected'&&<p className="mb-4 rounded-xl bg-[#fbf0dc] p-4 text-sm"><span className="font-semibold">店舗へ確認する食材：</span>{allergy.items.join('、')||'選択してください'}。予約前に、調理時の混入を含めて店舗へご確認ください。</p>}
          <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[12px] font-semibold tracking-[.12em] text-[#c03429]">RESTAURANTS · {query.area}</p><h2 className="jp-text mt-1 font-serif text-3xl font-semibold tracking-tight">{searched?candidates.length?`${query.purpose}に合う実店舗 ${candidates.length}件`:"検索結果":"条件を入力して実店舗を検索"}</h2></div><p className="max-w-md text-sm leading-6 text-[#6b635c]">予算、人数、個室などの条件から実在する店舗を比較します。空席とアレルギー対応は予約前に店舗へ確認してください。</p></div>
          ...
          {candidates.length>0&&<div className="grid gap-4 xl:grid-cols-3">
            {candidates.map((v,i)=><button key={v.id} aria-pressed={i===selected} onClick={()=>{setSelected(i);setCompleted(false);setStage("ranked")}} className={`group overflow-hidden rounded-[22px] border bg-white text-left transition duration-300 hover:-translate-y-1 hover:shadow-xl ${selected===i?"border-[#d03e28] ring-2 ring-[#d03e28]/12":"border-[#211f1d]/10"}`}>
              ...
              <div className="p-5"><p className="text-xs font-medium text-[#c03429]">{v.genre}</p><h3 className="mt-1 line-clamp-2 min-h-[56px] text-xl font-semibold tracking-tight">{v.name}</h3><div className="mt-3 flex items-start justify-between gap-3 text-sm"><span className="font-semibold">{v.budgetLabel}</span><span className="line-clamp-2 text-right text-xs text-[#6b635c]">{v.access}</span></div><p className="mt-4 min-h-[72px] text-sm leading-6 text-[#6b635c]">{v.reason}</p><div className="mt-4 grid grid-cols-3 gap-2 rounded-xl bg-[#fdf6ef] p-3"><ScorePart label="条件との一致" value={v.breakdown.fit}/><ScorePart label="予算" value={v.breakdown.budget}/><ScorePart label="利用しやすさ" value={v.breakdown.convenience}/></div><div className="mt-4 flex flex-wrap gap-2">{[v.privateRoom&&"個室あり",v.freeDrink&&"飲み放題",v.course&&"コースあり",v.partyCapacity&&`宴会最大${v.partyCapacity}名`].filter(Boolean).map(tag=><span key={String(tag)} className="rounded-full bg-[#f6ece1] px-2.5 py-1 text-xs text-[#6b635c]">{tag}</span>)}</div><div className="mt-5 flex items-center justify-between border-t border-[#211f1d]/8 pt-4"><span className="flex items-center gap-1.5 text-xs font-medium text-[#2f7d55]"><CircleCheck className="size-4"/>空席は店舗へ確認</span>{selected===i&&<span className="text-xs font-semibold text-[#d03e28]">選択中</span>}</div></div>
            </button>)}
          </div>}
      ...
      <aside className="lg:sticky lg:top-24 lg:self-start">
        <div id="plan-share" className="mb-4 scroll-mt-24 overflow-hidden rounded-[24px] border border-[#211f1d]/10 bg-[#d03e28] p-5 text-white shadow-[0_16px_40px_rgba(33,31,29,.12)]">
          <div className="flex items-center justify-between"><div><p className="text-xs font-semibold tracking-[.09em] text-[#f0c48a]">PLAN SHARE</p><h2 className="mt-1 text-xl font-semibold">みんなにプランを共有</h2></div><span className="grid size-10 place-items-center rounded-full bg-white/10"><Share2 className="size-5"/></span></div>
    ...
    <Dialog open={locationOpen} onOpenChange={setLocationOpen}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-[24px] bg-[#fffaf5] sm:max-w-[720px]"><DialogHeader><DialogTitle className="font-serif text-2xl">場所を選ぶ</DialogTitle><DialogDescription>検索したい都道府県を選択してください。全国47都道府県に対応しています。</DialogDescription></DialogHeader><div className="space-y-5 py-2">{PREFECTURE_REGIONS.map(region=><section key={region.name} aria-labelledby={`region-${region.name}`}><h3 id={`region-${region.name}`} className="mb-2 text-sm font-semibold text-[#6b635c]">{region.name}</h3><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">{region.prefectures.map(prefecture=>{const active=prefecture.code===prefectureCode;return <button key={prefecture.code} type="button" aria-pressed={active} onClick={()=>{setPrefectureCode(prefecture.code);setArea(prefecture.name);setLocationOpen(false)}} className={`flex min-h-11 items-center justify-between rounded-xl border px-3 text-left text-sm font-medium transition ${active?"border-[#d03e28] bg-[#d03e28] text-white shadow-sm":"border-[#211f1d]/10 bg-white text-[#211f1d] hover:border-[#d03e28]/40 hover:bg-[#f6ece1]"}`}><span>{prefecture.name}</span>{active&&<Check className="size-4"/>}</button>})}</div></section>)}</div><DialogFooter><Button variant="outline" onClick={()=>setLocationOpen(false)}>閉じる</Button></DialogFooter></DialogContent></Dialog>

    <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-[24px] bg-[#fffaf5] sm:max-w-[560px]"><DialogHeader><DialogTitle className="font-serif text-2xl">プランの詳細設定</DialogTitle><DialogDescription>日時や候補選びの優先条件を変更できます。</DialogDescription></DialogHeader><div className="space-y-5 py-2"><div className="grid grid-cols-2 gap-3"><div><Label className="mb-2 block">開催日</Label><Input type="date" value={draftEventDate} onChange={e=>setDraftEventDate(e.target.value)} className="h-11 bg-white"/></div><div><Label className="mb-2 block">開始時刻</Label><Input type="time" value={draftEventTime} onChange={e=>setDraftEventTime(e.target.value)} className="h-11 bg-white"/></div></div><div><Label className="mb-2 block">候補選びで優先すること</Label><Select value={draftPriority} onValueChange={v=>setDraftPriority(v as Priority)}><SelectTrigger className="h-11 w-full bg-white"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="balance">バランス</SelectItem><SelectItem value="conversation">会話しやすさ</SelectItem><SelectItem value="cost">予算の収まり</SelectItem><SelectItem value="access">アクセス情報</SelectItem></SelectContent></Select></div><SettingSwitch label="個室・半個室を優先" description="会話のしやすさを候補選びに加えます" checked={draftPrivateRoom} onCheckedChange={setDraftPrivateRoom}/><SettingSwitch label="食事制限の確認を追加" description="候補ごとに、予約前の店舗確認を案内します" checked={draftDietary} onCheckedChange={setDraftDietary}/><button type="button" onClick={()=>{setSettingsOpen(false);setAllergyOpen(true)}} className="flex min-h-14 w-full items-center justify-between rounded-2xl border border-[#211f1d]/10 bg-white px-4 text-left"><div><p className="text-sm font-semibold">食物アレルギー</p><p className="mt-1 text-xs text-[#6b635c]">{hasAllergy?`${allergy.items.join("、")}を確認`:allergy.status==="none"?"なし":"未設定"}</p></div><ChevronRight className="size-4 text-[#6b635c]"/></button></div><DialogFooter><Button variant="outline" onClick={()=>setSettingsOpen(false)}>変更しない</Button><Button onClick={applySettings} className="bg-[#d03e28] text-white hover:bg-[#b03320]">保存して候補を更新</Button></DialogFooter></DialogContent></Dialog>

    <Dialog open={allergyOpen} onOpenChange={setAllergyOpen}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-[24px] bg-[#fffaf5] sm:max-w-[680px]"><DialogHeader><DialogTitle className="font-serif text-2xl">アレルギーを設定</DialogTitle><DialogDescription>確認が必要な食材を選ぶと、プランと店舗への確認事項に反映されます。</DialogDescription></DialogHeader><div className="py-2"><AllergyPicker value={allergy} onChange={setAllergy} privateSharing={false}/></div><DialogFooter><Button onClick={()=>{setAllergyOpen(false);if(allergy.status==="selected")setDietary(true)}} className="bg-[#d03e28] text-white hover:bg-[#b03320]">設定を保存</Button></DialogFooter></DialogContent></Dialog>

    <Dialog open={approvalOpen} onOpenChange={setApprovalOpen}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-[24px] border-0 bg-[#fffaf5] p-0 sm:max-w-[600px]"><DialogHeader className="border-b border-[#211f1d]/10 p-6 text-left"><DialogTitle className="font-serif text-2xl">プランの最終確認</DialogTitle><DialogDescription>参加者へ共有する前に、日時・費用・確認事項をご確認ください。</DialogDescription></DialogHeader><div className="space-y-4 px-6"><div className="rounded-2xl border border-[#211f1d]/10 bg-white p-4"><p className="text-xs text-[#6b635c]">選択中の会場</p><p className="mt-1 text-lg font-semibold">{chosen?.name}</p><div className="mt-3 grid grid-cols-2 gap-3 text-sm"><span className="text-[#6b635c]">開催予定</span><span className="text-right font-medium">{eventDate} {eventTime}</span><span className="text-[#6b635c]">予算目安</span><span className="text-right font-medium">{chosen?.budgetLabel||"店舗へ確認"}</span><span className="text-[#6b635c]">定休日</span><span className="text-right font-medium">{chosen?.closed||"店舗ページで確認"}</span></div></div><div className="grid gap-3 sm:grid-cols-3"><CheckCard icon={Clock3} title="日程" value="開催日時を確認"/><CheckCard icon={UtensilsCrossed} title="食事" value={hasAllergy?`${allergy.items.length}項目を店舗へ確認`:"特記事項なし"}/><CheckCard icon={Users} title="参加者" value={`${query.people}名で共有`}/></div>{stage==="scheduled"?<div className="rounded-2xl bg-[#e6f3ec] p-4 text-sm text-[#2f7d55]"><div className="flex items-center gap-2 font-semibold"><CircleCheck className="size-5"/>カレンダー用ファイルを作成しました</div><p className="mt-1 pl-7 text-xs leading-5">端末のカレンダーへ追加できます。店舗への予約状況はグループで共有してください。</p></div>:completed?<div className="rounded-2xl bg-[#e6f3ec] p-4 text-sm text-[#2f7d55]"><div className="flex items-center gap-2 font-semibold"><CircleCheck className="size-5"/>{stage==="awaiting_approval"?"回答が揃い、最終確認待ちです":"参加者へ確認中です"}</div><p className="mt-1 pl-7 text-xs leading-5">出欠と希望条件をまとめて確認できます。</p></div>:<div className="rounded-2xl bg-[#f6ece1] p-4 text-xs leading-5 text-[#8a6a2f]">確定前に参加者へプランを共有し、出欠と食事に関する希望を確認しましょう。</div>}</div><DialogFooter className="p-6 pt-2 sm:justify-between"><Button variant="outline" onClick={()=>setApprovalOpen(false)}>候補を見直す</Button><Button onClick={advanceWorkflow} className="bg-[#d03e28] text-white hover:bg-[#b03320]">{stage==="scheduled"?<><Download className="mr-2 size-4"/>予定を再取得</>:stage==="awaiting_approval"?"プランを確定して予定作成":stage==="collecting"?"回答内容を確認":"参加者へ確認する"}</Button></DialogFooter></DialogContent></Dialog>
  </main></AppShell>;
}
...
function ScorePart({label,value}:{label:string;value:number}){return <div className="text-center"><p className="text-[10px] text-[#6b635c]">{label}</p><p className="mt-1 text-sm font-bold text-[#d03e28]">{value}</p></div>}
