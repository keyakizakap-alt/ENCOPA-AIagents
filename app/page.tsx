...
export default function Home() {
  ...
  return <main className="min-h-screen bg-[#f7f5ef] text-[#182523]">
    <header className="sticky top-0 z-40 border-b border-[#182523]/8 bg-white/95 shadow-[0_1px_0_rgba(24,37,35,.03)] backdrop-blur-xl lg:pl-[232px]">
      <div className="mx-auto flex h-[68px] max-w-[1400px] items-center justify-between px-4 sm:px-7 lg:px-8">
        <div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-[14px] bg-[#173f3a] text-[#fffdf7] shadow-[0_6px_18px_rgba(23,63,58,.16)]"><UtensilsCrossed className="size-[18px]"/></div><div><p className="text-[20px] font-black tracking-[.1em]">ENCOPA <span className="font-sans text-xs font-semibold tracking-normal text-[#65716e]">エンコパ</span></p><p className="hidden text-xs text-[#65716e] sm:block">お店選びから、みんなへの共有まで</p></div></div>
    ...
    <div className="lg:grid lg:grid-cols-[232px_minmax(0,1fr)]">
      <aside className="sticky top-0 hidden h-screen border-r border-white/10 bg-[#202322] px-4 py-5 text-white lg:flex lg:flex-col">
        ...
        <div className="mt-auto rounded-2xl border border-white/10 bg-white/[.04] p-4"><p className="text-sm font-semibold text-[#f3c765]">いい宴を、いいチームで。</p><p className="mt-2 text-xs leading-5 text-white/55">候補比較から参加者への共有まで、ひとつの流れで進められます。</p></div>
      </aside>
    ...
    <section className={`mx-auto grid max-w-[1400px] gap-6 px-4 py-5 sm:px-7 lg:px-8 lg:py-7 ${(activeView==="home"||activeView==="venues")?"lg:grid-cols-[minmax(0,1fr)_320px]":""}`}>
      <div className="min-w-0">
        ...
        {(activeView==="home"||activeView==="venues")&&<div className="overflow-hidden rounded-[26px] border border-[#182523]/9 bg-white shadow-[0_14px_40px_rgba(24,37,35,.07)]">
          <div className="grid gap-6 bg-[radial-gradient(circle_at_88%_0%,rgba(225,122,78,.13),transparent_20rem)] p-5 sm:p-7 xl:grid-cols-[1fr_270px] xl:p-8">
            <div className="self-center"><div className="mb-3 flex items-center gap-2 text-[#b55c38]"><Sparkles className="size-4"/><span className="text-[13px] font-semibold tracking-[.08em]">集まる日の準備を、ひとつに</span></div><h1 className="max-w-[700px] font-serif text-[clamp(2rem,4vw,3.35rem)] leading-[1.12] tracking-[-.04em] text-[#182523]">みんなが集まりやすい店を、<br className="hidden sm:block"/>迷わず決める。</h1><p className="mt-3 max-w-2xl text-base leading-7 text-[#65716e]">条件に合う実店舗を比べて、予約内容やアレルギー確認までひとつのプランにまとめます。</p></div>
            <div className="rounded-[22px] border border-[#182523]/8 bg-[#f7f5ef] p-5 text-[#182523]"><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-medium text-[#65716e]">今回の予算目安</p><p className="mt-1 text-3xl font-semibold tracking-tight">{total.toLocaleString()}<span className="ml-1 text-base font-medium text-[#65716e]">円</span></p></div><WalletCards className="size-5 text-[#b55c38]"/></div><div className="mt-5 grid grid-cols-3 gap-2 border-t border-[#182523]/8 pt-4 text-center"><LightPlanFact label="開催" value={`${eventDate.slice(5).replace("-","/")} ${eventTime}`}/><LightPlanFact label="優先" value={priorityLabels[priority]}/><LightPlanFact label="食事" value={hasAllergy?`${allergy.items.length}項目`:allergy.status==="none"?"指定なし":"未設定"}/></div></div>
          </div>
          <div className="grid gap-3 border-t border-[#182523]/8 bg-white p-4 sm:grid-cols-2 sm:p-5 xl:grid-cols-[.9fr_1.15fr_.68fr_.56fr_.85fr_auto]">
            <Field label="目的"><Select value={purpose} onValueChange={setPurpose}><SelectTrigger className="h-12 w-full rounded-[14px] border-[#173f3a]/12 bg-white text-[#182523] shadow-sm"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="忘年会">忘年会</SelectItem><SelectItem value="新年会">新年会</SelectItem><SelectItem value="歓迎会">歓迎会</SelectItem><SelectItem value="送別会">送別会</SelectItem><SelectItem value="懇親会">懇親会</SelectItem><SelectItem value="打ち上げ">打ち上げ</SelectItem></SelectContent></Select></Field>
            <Field label="場所"><button type="button" onClick={()=>setLocationOpen(true)} className="flex h-12 w-full items-center justify-between rounded-[14px] border border-[#173f3a]/12 bg-white px-3 text-left text-[#182523] shadow-sm transition hover:border-[#47766f]/50 hover:bg-[#fafbf8]"><span className="flex min-w-0 items-center gap-2"><MapPin className="size-4 shrink-0 text-[#47766f]"/><span className="truncate text-base font-semibold">{area}</span></span><ChevronRight className="size-4 shrink-0 text-[#7d8986]"/></button></Field>
            <Field label="予算 / 人"><div className="relative"><Input aria-label="1人あたりの予算" inputMode="numeric" value={budget} onChange={e=>setBudget(e.target.value.replace(/\D/g,""))} className="h-12 rounded-[14px] border-[#173f3a]/12 bg-white pr-9 text-base shadow-sm"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-[#65716e]">円</span></div></Field>
            <Field label="人数"><div className="relative"><Input aria-label="参加人数" inputMode="numeric" value={people} onChange={e=>setPeople(e.target.value.replace(/\D/g,""))} className="h-12 rounded-[14px] border-[#173f3a]/12 bg-white pr-9 text-base shadow-sm"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-[#65716e]">名</span></div></Field>
            ...
            <div className="flex items-end"><Button disabled={searching} onClick={()=>void search()} className="h-12 w-full rounded-[14px] bg-[#df7549] px-6 text-base font-bold text-white shadow-[0_10px_22px_rgba(197,89,48,.24)] hover:bg-[#c96139] xl:w-auto">{searching?<RefreshCw className="mr-2 size-4 animate-spin"/>:<Search className="mr-2 size-4"/>}{searching?"検索中":"お店を探す"}</Button></div>
          </div>
        </div>}
        ...
        {activeView==="venues"&&<section id="results" className="scroll-mt-24 pt-9">
          ...
          {candidates.length>0&&<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {candidates.map((v,i)=><button key={v.id} aria-pressed={selected===i} onClick={()=>{setSelected(i);setCompleted(false);setStage("ranked")}} className={`group overflow-hidden rounded-[24px] border bg-white text-left transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(24,37,35,.11)] ${selected===i?"border-[#173f3a] shadow-[0_16px_40px_rgba(23,63,58,.12)] ring-2 ring-[#173f3a]/10":"border-[#182523]/8 shadow-[0_8px_24px_rgba(24,37,35,.05)]"}`}>
              <div className="relative h-44 overflow-hidden bg-gradient-to-br from-[#436d72] to-[#264f57] text-white">{v.photoUrl&&<Image src={v.photoUrl} alt="" fill sizes="(min-width:1280px) 30vw, (min-width:768px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-[1.04]"/>}<div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/5 to-black/10"/><div className="absolute inset-x-0 top-0 flex items-start justify-between p-4"><Badge className={`${i===0?"bg-[#df7549]":"bg-black/40"} border-white/15 text-white shadow-sm`}>{i===0?"おすすめ":`候補 ${i+1}`}</Badge><div className="rounded-full bg-white px-3 py-1.5 text-[#173f3a] shadow-lg"><span className="text-base font-black">{v.score}</span><span className="ml-0.5 text-[10px] font-semibold text-[#65716e]">/100</span></div></div>{selected===i&&<span className="absolute bottom-3 right-3 flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-bold text-[#173f3a] shadow-lg"><Check className="size-3.5"/>選択中</span>}</div>
              <div className="p-5"><p className="text-xs font-bold tracking-wide text-[#b55c38]">{v.genre}</p><h3 className="mt-1 line-clamp-2 min-h-[56px] text-xl font-bold tracking-tight">{v.name}</h3><div className="mt-3 flex items-start justify-between gap-3 text-sm"><span className="shrink-0 font-bold text-[#173f3a]">{v.budgetLabel}</span><span className="line-clamp-2 text-right text-xs leading-5 text-[#65716e]">{v.access}</span></div><p className="mt-4 min-h-[72px] text-sm leading-6 text-[#65716e]">{v.reason}</p><div className="mt-4 grid grid-cols-3 gap-2 rounded-2xl bg-[#f5f3ed] p-3"><ScorePart label="条件" value={v.breakdown.fit}/><ScorePart label="予算" value={v.breakdown.budget}/><ScorePart label="使いやすさ" value={v.breakdown.convenience}/></div><div className="mt-4 flex flex-wrap gap-2">{[v.privateRoom&&"個室あり",v.freeDrink&&"飲み放題",v.course&&"コースあり",v.partyCapacity&&`最大${v.partyCapacity}名`].filter(Boolean).map(tag=><span key={String(tag)} className="rounded-full border border-[#173f3a]/8 bg-[#edf3ef] px-2.5 py-1 text-xs font-medium text-[#3f5f58]">{tag}</span>)}</div><div className="mt-5 flex items-center justify-between border-t border-[#182523]/8 pt-4"><span className="flex items-center gap-1.5 text-xs font-semibold text-[#2f6b57]"><CircleCheck className="size-4"/>空席は店舗へ確認</span><span className="text-xs font-semibold text-[#77807e]">詳細を見る</span></div></div>
            </button>)}
          </div>}
          ...
          <div className="mt-5 grid gap-4 rounded-[26px] border border-[#173f3a]/12 bg-[#e8f0ec] p-5 sm:p-6 xl:grid-cols-[1.1fr_.9fr_auto] xl:items-center"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-white text-[#173f3a] shadow-sm"><Users className="size-4"/></span><div><p className="font-bold">このプランで参加者に確認する</p><p className="mt-1 text-sm text-[#65716e]">日時・候補店・予算をまとめて共有できます</p></div></div><div className="grid grid-cols-3 gap-3 text-center"><MiniStat label="参加予定" value={`${query.people}名`}/><MiniStat label="予算目安" value={chosen.budgetLabel}/><MiniStat label="候補順位" value={`${selected+1}位`}/></div><Button onClick={()=>setApprovalOpen(true)} className="h-12 rounded-[14px] bg-[#173f3a] px-6 font-bold text-white shadow-lg hover:bg-[#0e332f]">内容を確認<ArrowRight className="ml-2 size-4"/></Button></div></>}
        </section>}

        {activeView==="home"&&<section className="pt-6"><div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]"><div className="rounded-[26px] border border-[#182523]/8 bg-white p-5 shadow-[0_12px_34px_rgba(24,37,35,.05)] sm:p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold tracking-[.12em] text-[#b55c38]">CURRENT PLAN</p><h2 className="mt-1 text-2xl font-bold">{query.purpose} {eventDate.slice(0,4)}</h2></div><Badge className="bg-[#e8f0ec] text-[#2f6b57]">候補を比較中</Badge></div><div className="mt-6 grid gap-4 sm:grid-cols-3"><SummaryFact icon={CalendarDays} label="開催" value={`${eventDate} ${eventTime}`}/><SummaryFact icon={Users} label="人数" value={`${query.people}名`}/><SummaryFact icon={WalletCards} label="予算 / 人" value={`${query.budget.toLocaleString()}円`}/><SummaryFact icon={MapPin} label="場所" value={query.area}/><SummaryFact icon={Search} label="候補" value={candidates.length?`${candidates.length}件を比較中`:"未検索"}/><SummaryFact icon={UtensilsCrossed} label="食事の配慮" value={hasAllergy?`${allergy.items.length}項目`:allergy.status==="none"?"なし":"未設定"}/></div><Button onClick={()=>navigate("venues")} className="mt-6 h-11 rounded-xl bg-[#df5542] px-5 text-white hover:bg-[#c84636]">会場候補を確認<ArrowRight className="ml-2 size-4"/></Button></div><div className="rounded-[26px] border border-[#182523]/8 bg-white p-5 shadow-[0_12px_34px_rgba(24,37,35,.05)] sm:p-6"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">やることリスト</h2><button type="button" onClick={()=>navigate("history")} className="text-sm font-semibold text-[#b55c38]">履歴を見る</button></div><div className="mt-5 space-y-2"><TodoRow done={candidates.length>0} label="会場候補を確認する" onClick={()=>navigate("venues")}/><TodoRow done={stageIndex>2} label="参加者に連絡する" onClick={()=>navigate("participants")}/><TodoRow done={stage==="scheduled"} label="予約内容を確定する" onClick={()=>setApprovalOpen(true)}/></div></div></div></section>}

        {activeView==="participants"&&<section><ScreenHeading eyebrow="PARTICIPANTS" title="参加者と連絡" description="予約内容、出欠、アレルギー確認、グループチャットをまとめて管理します。"/>{chosen?<><div className="grid gap-5 lg:grid-cols-[.75fr_1.25fr]"><div className="rounded-[24px] border border-[#182523]/8 bg-white p-5"><p className="text-xs font-bold text-[#b55c38]">共有する予約内容</p><h2 className="mt-2 text-xl font-bold">{chosen.name}</h2><p className="mt-2 text-sm leading-6 text-[#65716e]">{chosen.address}</p><div className="mt-4"><MapLinks address={chosen.address}/></div><div className="mt-5 grid grid-cols-2 gap-3"><MiniStat label="日時" value={`${eventDate.slice(5).replace("-","/")} ${eventTime}`}/><MiniStat label="人数" value={`${query.people}名`}/></div></div><div className="rounded-[24px] border border-[#182523]/8 bg-[#fff8f2] p-5"><div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#df5542] text-white"><MessageCircle className="size-5"/></span><div><h2 className="text-xl font-bold">グループを作成して共有</h2><p className="mt-2 text-sm leading-6 text-[#65716e]">作成後は専用画面へ移動し、参加者ごとの予約確認とグループチャットを利用できます。</p></div></div></div></div><CreateGroup title={`${query.purpose}のグループ`} initial={{venueName:chosen.name,address:chosen.address,date:eventDate,time:eventTime,people:query.people,price:chosen.estimatedPrice||query.budget,status:"planning",bookingReference:"",note:"",website:chosen.url}}/></>:<EmptyScreen icon={Users} title="先に会場候補を選んでください" description="参加者へ共有する店舗を選ぶと、グループを作成できます。" action="会場候補へ" onClick={()=>navigate("venues")}/>}</section>}
...
function Field({label,children}:{label:string;children:React.ReactNode}){return <div><Label className="mb-2 block text-[13px] font-bold text-[#4e5b57]">{label}</Label>{children}</div>}
function LightPlanFact({label,value}:{label:string;value:string}){return <div className="min-w-0"><p className="text-[11px] font-medium text-[#7b8582]">{label}</p><p className="mt-1 truncate text-xs font-bold text-[#263532]">{value}</p></div>}
function MiniStat({label,value}:{label:string;value:string}){return <div><p className="text-xs text-[#73807c]">{label}</p><p className="mt-1 text-sm font-bold">{value}</p></div>}
function SummaryFact({icon:Icon,label,value}:{icon:React.ElementType;label:string;value:string}){return <div className="flex min-w-0 items-center gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-[#f3f0e8] text-[#1f4b46]"><Icon className="size-5"/></span><div className="min-w-0"><p className="text-xs font-semibold text-[#87908d]">{label}</p><p className="mt-1 truncate font-bold">{value}</p></div></div>}
function TodoRow({done,label,onClick}:{done:boolean;label:string;onClick:()=>void}){return <button type="button" onClick={onClick} className="flex min-h-14 w-full items-center gap-3 rounded-2xl px-3 text-left transition hover:bg-[#f7f5ef]"><span className={`grid size-6 shrink-0 place-items-center rounded-full ${done?"bg-[#3f8a70] text-white":"border-2 border-[#ccd1cd] text-transparent"}`}><Check className="size-3.5"/></span><span className={`flex-1 text-sm font-semibold ${done?"text-[#52605c]":"text-[#182523]"}`}>{label}</span><ChevronRight className="size-4 text-[#9ca4a1]"/></button>}
...
function ScorePart({label,value}:{label:string;value:number}){return <div className="text-center"><p className="text-xs text-[#73807c]">{label}</p><p className="mt-1 text-base font-black text-[#173f3a]">{value}</p></div>}
