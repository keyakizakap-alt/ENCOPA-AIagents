        ...
        {activeView==="venues"&&<section id="results" className="scroll-mt-24 pt-9">
          ...
          {candidates.length>0&&<div className="grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(min(100%,280px),1fr))]">
            {candidates.map((v,i)=><article key={v.id} className={`group overflow-hidden rounded-[24px] border bg-white text-left transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(24,37,35,.11)] ${selected===i?"border-[#173f3a] shadow-[0_16px_40px_rgba(23,63,58,.12)] ring-2 ring-[#173f3a]/10":"border-[#182523]/8 shadow-[0_8px_24px_rgba(24,37,35,.05)]"}`}>
              ...
              <div className="p-5"><p className="text-xs font-bold tracking-wide text-[#b55c38]">{v.genre}</p><h3 className="mt-1 line-clamp-2 min-h-[56px] text-xl font-bold tracking-tight">{v.name}</h3><div className="mt-3 flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 text-sm"><span className="whitespace-nowrap font-bold text-[#173f3a]">{v.budgetLabel}</span><span className="jp-text line-clamp-2 min-w-0 text-xs leading-5 text-[#65716e]">{v.access}</span></div><p className="mt-4 min-h-[72px] text-sm leading-6 text-[#65716e]">{v.reason}</p><div className="mt-4 grid grid-cols-3 gap-2 rounded-2xl bg-[#f5f3ed] p-3"><ScorePart label="条件" value={v.breakdown.fit}/><ScorePart label="予算" value={v.breakdown.budget}/><ScorePart label="利便" value={v.breakdown.convenience}/></div><div className="mt-4 flex flex-wrap gap-2">{[v.privateRoom&&"個室あり",v.freeDrink&&"飲み放題",v.course&&"コースあり",v.partyCapacity&&`最大${v.partyCapacity}名`].filter(Boolean).map(tag=><span key={String(tag)} className="rounded-full border border-[#173f3a]/8 bg-[#edf3ef] px-2.5 py-1 text-xs font-medium text-[#3f5f58]">{tag}</span>)}</div><div className="mt-5 border-t border-[#182523]/8 pt-4"><span className="flex items-center gap-1.5 whitespace-nowrap text-xs font-semibold text-[#2f6b57]"><CircleCheck className="size-4 shrink-0"/>空席は店舗へ確認</span><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" aria-pressed={selected===i&&picked} onClick={()=>{setSelected(i);setPicked(true);setCompleted(false);setStage("ranked")}} className={`min-h-11 rounded-xl px-3 text-sm font-bold transition ${selected===i&&picked?"bg-[#e4eee9] text-[#24564d]":"border border-[#173f3a]/15 bg-white text-[#173f3a] hover:bg-[#f3f7f4]"}`}>{selected===i&&picked?"選択済み":"この候補を選ぶ"}</button><a href={v.url} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-[#173f3a] px-3 text-sm font-bold text-white transition hover:bg-[#0e332f]">詳細を見る<ExternalLink className="size-3.5"/></a></div></div></div>
            </article>)}
          </div>}
...
function MiniStat({label,value}:{label:string;value:string}){return <div className="min-w-0"><p className="whitespace-nowrap text-xs text-[#73807c]">{label}</p><p className="jp-text mt-1 text-[13px] font-bold sm:text-sm">{value}</p></div>}
