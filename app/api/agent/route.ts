...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const specialistResults = await Promise.allSettled(
      specialistPrompts.map((agent) => callOrca(apiKey, `${agent.role}です。${sharedRules}${agent.task}`, context, 550)),
    );
    const completed = specialistResults.flatMap((result) => result.status === "fulfilled" ? [result.value] : []);
    if (completed.length < 2) throw new HttpError(502, "候補分析が混み合っています。少し待って再度お試しください。");

    const synthesis = await callOrca(
      apiKey,
      ...
      { context, specialistResults: completed.map((result) => result.value) },
      850,
    );

    const plan = normalizePlan(synthesis.value, candidates, traceId, [...completed, synthesis].map((result) => result.resolvedModel));
...
function normalizePlan(value: Record<string, unknown>, candidates: Candidate[], traceId: string, models: string[]): AgentPlan {
