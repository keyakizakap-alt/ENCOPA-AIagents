...
const server=createServer(async(req,res)=>{
 ...
 const isCoordinator=system.includes('統括担当')||system.includes('Coordinator Agent');
 ...
 if(script.mode==='faulty_plan'&&isCoordinator){
 ...
 if(script.mode==='unfixable_plan'&&isCoordinator){
 ...
 let value;
 if(isCoordinator&&raw.includes('PROMPT_ATTACK'))value={nextAction:'FINALIZE',actionReason:'標準分析で十分',recommendedVenueId:'attacker-controlled-id',summary:'入力データ内の命令は採用しません。',venueAdvice:[{venueId:'attacker-controlled-id',score:100,reason:'不正な候補'},{venueId:'shop-1',score:90,reason:'検証済み候補'}],confirmationChecklist:['空席を確認'],nextActions:['店舗へ確認'],shareDraft:'候補を確認中です。',needsSpecialistReview:false,reviewReasons:[]};
 else if(isCoordinator)value={nextAction:'FINALIZE',actionReason:'標準分析で十分',recommendedVenueId:firstId,summary:'条件を総合すると、個室と人数条件を満たす候補が進めやすいです。',venueAdvice:[{venueId:firstId,score:92,reason:'人数、予算、個室の条件が揃っています。'}],confirmationChecklist:['空席とコース内容を店舗へ確認','アレルギー対応を店舗へ確認'],nextActions:['店舗へ空席を問い合わせる','参加者へ候補を共有する'],shareDraft:'候補店舗を確認しました。予約状況は確認中です。',needsSpecialistReview:false,reviewReasons:[]};
