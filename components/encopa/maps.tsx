export function MapLinks({address}:{address:string}){
  if(!address.trim())return <p className="text-sm text-[#687370]">住所を入力すると地図と経路を確認できます。</p>;
...
export function VenueMap({address,label}:{address:string;label:string}){
  ...
  if(!location)return <div className="grid min-h-48 place-items-center rounded-2xl bg-[#eef0eb] px-6 text-center text-sm leading-6 text-[#687370]">住所を入力すると、ここに周辺地図が表示されます。</div>;
  return <iframe
    title={`${label}の周辺地図`}
