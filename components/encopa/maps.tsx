...
export function VenueMap({address,label}:{address:string;label:string}){
  ...
  if(!location)return <div className="grid min-h-48 place-items-center rounded-2xl bg-[#f6ece1] px-6 text-center text-sm leading-6 text-[#6b635c]">住所を入力すると、ここに周辺地図が表示されます。</div>;
  return <iframe
    title={`${label}の周辺地図`}
